...
type WalkFunc func(workflow.Context, *DagGraph, workflow.CancelFunc, Vertex, *BoolStruct, map[Vertex]bool) (bool, tfdiags.Diagnostics)
...
// Roots returns the roots of the DAG, or an error.
//
// Complexity: O(V)
func (g *AcyclicGraph) Roots() ([]Vertex, error) {
	roots := make([]Vertex, 0, 1)
	for _, v := range g.Vertices() {
		if g.UpEdges(v).Len() == 0 {
			roots = append(roots, v)
...
// with no cycles.
// we are supporting multiple roots right now
...
func (g *AcyclicGraph) Walk(ctx workflow.Context, cancelFunc workflow.CancelFunc, cb WalkFunc) tfdiags.Diagnostics {
	defer g.debug.BeginOperation(typeWalk, "").End("")

	w := &Walker{Callback: cb, Reverse: true, VisitedNodes: make(map[Vertex]bool)}
	w.Update(ctx, cancelFunc, &DagGraph{Graph:g})

	completed := false
	var ret tfdiags.Diagnostics
	for !completed {
		_, ret, completed = w.Wait(ctx)
	}
